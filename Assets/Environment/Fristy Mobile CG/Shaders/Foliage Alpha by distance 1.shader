...
Shader "Fristy/Nature/Leaves Distance"
{
    Properties
    {
...
void Unity_GradientNoise_float(float2 UV, float Scale, out float Out)
{
    ...
    void Unity_TilingAndOffset_float(float2 UV, float2 Tiling, float2 Offset, out float2 Out)
    {
       ...
       struct Input
		...
		void vert (inout appdata_full v, out Input o) {
            ...
            o.posInSpace =  mul(unity_ObjectToWorld, v.vertex);
          ...
          float lvl_ = smoothstep(-0.55, 0,-v.color.r);
          
           

            v.vertex.xz += lerp(final, 0, lvl_);
	...
	float3 SubsurfaceShadingSimple(float3 diffColor, float3 normal, float3 viewDir, float3 thickness, float3 lightDir, float3 lightColor)
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
         ...
         float dist = distance(IN.posInSpace, -_WorldSpaceCameraPos);
